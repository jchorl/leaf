...
If not using docker, just `make nodocker` and hopefully it will work (I can't test this because I don't have node installed)

If using docker, `make`.

Troubleshooting:
`npm start` actually starts webservers that listen on ports 19000 and 19001. It also grabs the ip address of the local system and generates a QR code based on that ip. Unfortunately, the ip it grabs within the docker container is not the ip of the host and is not accessible on the local network. So the ip of the host is passed in as an env variable because it is accessible on the local network. The host ip is picked up using a shell command at the top of the makefile. If that ip is incorrect, try adjusting the command in the makefile to
properly grab your ip and make sure that env variable gets set properly.
